...
export default function Page() {
  ...
  const fetchMarketplace = async (page = 1, category = categoriesSelect) => {
    ...
    try {
      let query = `?action_type=service&action_for=customer&page=${page}&limit=${pagination.limit}`;
            ...
            {search && (
              <div className="max-w-[1400px] mx-auto px-4 mt-2">
                <p className="text-sm text-gray-500">
                  Searching for <span className="font-medium text-gray-700">{search}</span> in Services
                </p>
              </div>
            )}
          </div>
                ...
                <div className="w-11/12 m-auto mt-20">
                  <div className="max-w-[400px]">
                    ...
                    <div className="text-neutral-500 text-left text-sm sm:text-base">
                      Expert-led services — fully private offer aligned to your AWS MACC or Azure EDP commitments.
                    </div>
                  </div>
                </div>
              )}
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-4 mb-8">
                    <div>
                      <h2 className="text-2xl font-bold text-gray-800">
                        {search ? `Search Results for "${search}"` : `Filtered Services`}
                      </h2>
                      <p className="text-gray-600 mt-2">
                        Showing {marketplace.length} services
                  ...
                  <div className="col-span-full text-center py-16">
                    ...
                    <p className="text-gray-500 mb-6">
                      {search 
                        ...
                        : `No services available${categoriesSelect ? ` in "${categoriesSelect}"` : ''}`
                      }
                    </p>
                    {(search || categoriesSelect) && (
                      <button
              ...
              {marketplace.length > 0 && pagination.pages > 1 && (
                <div className="flex flex-col items-center justify-center mt-10 space-y-4">
                  ...
                  <div className="text-gray-600 text-sm">
                    ...
                    {pagination.total > 0 && ` (${pagination.total} total services)`}
                  </div>
                </div>
              )}
            </>
