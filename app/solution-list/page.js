...
export default function Page() {
  ...
  const fetchMarketplace = async (page = 1, category = categoriesSelect) => {
    ...
    try {
      let query = `?action_type=solutions&action_for=customer&page=${page}&limit=${pagination.limit}`;
            ...
            {search && (
              <div className="max-w-[1400px] mx-auto px-4 mt-2">
                <p className="text-sm text-gray-500">
                  Searching for <span className="font-medium text-gray-700">{search}</span> in Solutions
                </p>
              </div>
            )}
          </div>
                ...
                <div className="w-11/12 m-auto mt-20">
                  <div className="max-w-[400px]">
                    <div className="text-black font-['CreatoDisplay-Regular',_sans-serif] text-left text-xl sm:text-2xl mb-5 md:text-[25px] font-normal">
                      Popular & Trending Solutions
                    </div>
                    <div className="text-neutral-500 text-left text-sm sm:text-base">
                      Ready-to-deploy solutions — fully private offer aligned to your AWS MACC or Azure EDP commitments.
                    </div>
                  </div>
                </div>
              )}
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-4 mb-8">
                    <div>
                      <h2 className="text-2xl font-bold text-gray-800">
                        {search ? `Search Results for "${search}"` : `Filtered Solutions`}
                      </h2>
                      <p className="text-gray-600 mt-2">
                        Showing {marketplace.length} solutions
              ...
              <div className="w-11/12 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 m-auto xl:grid-cols-4 gap-5 mt-5">
                {Array.isArray(marketplace) && marketplace.length > 0 ? (
                    ...
                    >
                      <div className="bg-zinc-50 border border-zinc-200 m-auto h-[420px] w-full max-w-[295px] hover:shadow-lg transition-shadow">
                        <div className="w-full h-[250px] relative">
                          <Image
                            fill
                            alt={product.title || "Solution Image"}
                  ...
                  <div className="col-span-full text-center py-16">
                    ...
                    <h3 className="text-xl font-semibold text-gray-600 mb-2">No Solutions Found</h3>
                    <p className="text-gray-500 mb-6">
                      {search 
                        ? `We couldn't find any solutions matching "${search}"`
                        : `No solutions available${categoriesSelect ? ` in "${categoriesSelect}"` : ''}`
                      }
                    </p>
                    {(search || categoriesSelect) && (
                      <button
              ...
              {marketplace.length > 0 && pagination.pages > 1 && (
                <div className="flex flex-col items-center justify-center mt-10 space-y-4">
                  ...
                  <div className="text-gray-600 text-sm">
                    ...
                    {pagination.total > 0 && ` (${pagination.total} total solutions)`}
                  </div>
                </div>
              )}
            </>
